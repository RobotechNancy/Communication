...
/*!
    ...
    \param Stopbit: Number of stop bits
     ...
     \return 500 success
     \return -501 device not found
     \return -502 error while opening the device
     \return -503 error while getting port parameters
     \return -504 Speed (Bauds) not recognized
     \return -505 error while writing port parameters
     \return -506 error while writing timeout parameters
     \return -507 Databits not recognized
     \return -508 Stopbits not recognized
     \return -509 Parity not recognized
  */
int serialib::openDevice(const char *Device, const unsigned int Bauds,
    ...
    if(hSerial==INVALID_HANDLE_VALUE) {
        if(GetLastError()==ERROR_FILE_NOT_FOUND)
            return XB_E_SER_NOT_FOUND; // Device not found

        // Error while opening the device
        return XB_E_SER_OPEN;
    }
    ...
    if (!GetCommState(hSerial, &dcbSerialParams)) return XB_E_SER_PARAM;
    ...
    default : return XB_E_SER_UKN_BAUDRATE;
    }
    //select data size
    BYTE bytesize = 0;
    switch(Databits) {
        ...
        default: return XB_E_SER_UKN_DATABITS;
    }
    BYTE stopBits = 0;
    switch(Stopbits) {
        ...
        default: return XB_E_SER_UKN_STOPBITS;
    }
    BYTE parity = 0;
    switch(Parity) {
        ...
        default: return XB_E_SER_UKN_PARITY;
    }
    // configure byte size
    ...
    if(!SetCommState(hSerial, &dcbSerialParams)) return XB_E_SER_CONFIG;
    ...
    // Write the parameters
    if(!SetCommTimeouts(hSerial, &timeouts)) return XB_E_SER_TIMOUT;
